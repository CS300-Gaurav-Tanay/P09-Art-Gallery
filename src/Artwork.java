...
public class Artwork implements Comparable<Artwork> {
   ...
   * @throws IllegalArgumentException if year is less than 1000, if cost is negative or zero or if
   ...
   * artwork; an integer less then 0 if this artwork is older than the otherArtwork, or if they
   * were created in the same year but this artwork is less expensive than the otherArtwork, or if
   * they have the same year and the same cost and this artwork has a lower lexical order name
   * than otherArtwork name; String comparisons are case-sensitive. otherwise returns an integer
   * greater than 0.
   */
  @Override public int compareTo(Artwork otherArtwork) {
    // TODO complete the implementation of this method
    if (this.getYear() < otherArtwork.getYear()) {
      // this is older than otherArtwork so this is smaller
      return -1;
    } else if (this.getYear() > otherArtwork.getYear()) {
      return 1;
    } else {
      // both this and otherArtwork were made in same year so comparing cost
      if (this.getCost() < otherArtwork.getCost()) {
        ...
        // both cost and name of this and otherArtwork is same so using String.compareTo()
